...
public class AppointmentDaoImpl implements AppointmentDao{
	...
	public List<AppoinmentDTO> findTodaysAppoinmentsById(LocalDate date, Long labId, LocalDate fromDate,
		...
		String sql = " SELECT sa.id AS appointmentId, ud.username AS patientName,"
				...
				+ " MAX(sa.app_date) AS appointmentDate, MAX(sa.shop_id) AS shopId" + " FROM appointment_details AS ap"
				+ " JOIN user_details AS ud ON ud.id = ap.consumer_id"
				+ " JOIN slot_availibility AS sa ON sa.id = ap.slot_id"
				+ " WHERE sa.shop_id = :labId AND sa.app_date = :appDate"
				+ " GROUP BY sa.id, ud.username, ud.adhar_photo, ud.phone, ap.appointment_status";

		Query query = entityManager.createNativeQuery(sql);
		query.setParameter("labId", 1);
		query.setParameter("appDate", "2023-12-19");
		...
		List<AppoinmentDTO> result = query.getResultList();
